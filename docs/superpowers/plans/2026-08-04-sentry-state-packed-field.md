...
**Architecture:** `ReceiveFrame` and `GimbalState` store the wire value as `sentry_state`. Protocol helpers encode/decode the upper two mode bits and the lower fourteen state bits. `Gimbal`, `Aim2Nav`, and the raw-debug tool consume the helpers instead of independent numeric mappings.
...
- Bits 15:14 are mode: `00` idle, `01` auto aim, `10` small buff, `11` big buff.
- Bits 13:0 are a 14-bit sentry status value.
...
```cpp
expect_equal_hex(pack_sentry_state(0x1234, GimbalMode::BIG_BUFF), 0xD234, ...);
expect_equal_hex(sentry_status(0xD234), 0x1234, ...);
expect_true(sentry_mode(0xD234) == GimbalMode::BIG_BUFF, ...);
```
...
expect_equal_hex(parsed_state->sentry_state, 0x5234, ...);
expect_true(bytes[35] == 0x34 && bytes[36] == 0x52, ...);
```
