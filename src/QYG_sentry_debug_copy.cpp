  ...
  auto plan_thread = std::thread([&]() {
    while (!quit && rclcpp::ok()) {
      ...
      if (mode.load() == io::GimbalMode::AUTO_AIM && !target_queue.empty()) {
        ...
        // // gimbal.send(plan.control,plan.fire,0.00,0.00);
        // tools::logger()->info("Sent Command - Control: {}, Fire: {}, Yaw: {:.2f}, Pitch: {:.2f}, NYAW: {:.2f}, NPITCH: {:.2f}, MODE: {},navx:{},navy:{},navz:{}",
        //   plan.control, plan.fire, plan.yaw, plan.pitch,gs.yaw_imu,gs.pitch_imu,gs.mode,cmd_vel.linear.x,cmd_vel.linear.y, cmd_vel.angular.z);
