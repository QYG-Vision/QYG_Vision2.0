  ...
  std::vector<std::string> & img_paths)
{
  ...
  for (int i = 1; true; i++) {
    ...
    params.minCircularity = 0.2; // 原本默认通常较高，放宽到 0.2
    params.filterByConvexity = true;
    params.minConvexity = 0.2;    // 放宽到 0.2
    params.filterByInertia = true;
    params.minInertiaRatio = 0.1; // 放宽到 0.1
    
    cv::Ptr<cv::FeatureDetector> blobDetector = cv::SimpleBlobDetector::create(params);
    auto success = cv::findCirclesGrid(
      img, pattern_size, centers_2d, cv::CALIB_CB_SYMMETRIC_GRID, blobDetector);

    // 显示识别结果
    auto drawing = img.clone();
    if (success) {
      for (size_t k = 0; k < centers_2d.size(); k++) {
        cv::circle(drawing, centers_2d[k], 5, {0, 255, 0}, 2);
        cv::putText(drawing, std::to_string(k), centers_2d[k], cv::FONT_HERSHEY_SIMPLEX, 0.4, {0, 0, 255}, 1);
      }
    } else {
        // 如果失败，尝试画出所有检测到的斑点，帮助调试
        std::vector<cv::KeyPoint> keypoints;
        blobDetector->detect(img, keypoints);
        for (const auto& kp : keypoints) {
            cv::circle(drawing, kp.pt, kp.size/2, {0, 0, 255}, 1);
        }
        cv::putText(drawing, "Detection Failed", {50, 50}, cv::FONT_HERSHEY_SIMPLEX, 1, {0, 0, 255}, 2);
    }
    cv::resize(drawing, drawing, {}, 0.5, 0.5);  // 缩小显示
    cv::imshow("Calibration Process", drawing);
    cv::waitKey(success ? 100 : 500); // 识别成功跳过快一点，失败停留久一点
...
int main(int argc, char * argv[])
{
  ...
  // 迭代终止条件：圆点阵列精度约为0.5像素，使用适当的精度阈值
  auto criteria = cv::TermCriteria(
    cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 100, 1e-6);
  
  // 标定标志：圆点阵列检测精度稍低于棋盘格，保持灵活性
