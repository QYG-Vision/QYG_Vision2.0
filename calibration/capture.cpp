  ...
  const std::string & config_path, const std::string & can, const std::string & output_folder)
{
  ...
  while (true) {
    ...
    Eigen::Vector3d euler_deg = gimbal.euler(timestamp);
    Eigen::Vector3d euler_rad = euler_deg * (CV_PI / 180.0);
    ...
    // --- 显示原始欧拉角 (对比用) ---
    Eigen::Vector3d raw_deg = euler_deg;
    tools::draw_text(img_with_ypr, fmt::format("Raw_Yaw(Z) {:.2f}", raw_deg[2]), {350, 40}, {0, 255, 0});
    tools::draw_text(img_with_ypr, fmt::format("Raw_Pitch(Y) {:.2f}", raw_deg[1]), {350, 80}, {0, 255, 0});
    tools::draw_text(img_with_ypr, fmt::format("Raw_Roll(X) {:.2f}", raw_deg[0]), {350, 120}, {0, 255, 0});
    // 4. 显示新增的关键调试信息
    ...
    // 6. 识别圆点阵列标定板 (供实时确认 - 严谨模式)
    std::vector<cv::Point2f> centers_2d;
    bool detect_success = cv::findCirclesGrid(img, pattern_size, centers_2d, cv::CALIB_CB_SYMMETRIC_GRID);
    
    if (detect_success) {
      for (size_t k = 0; k < centers_2d.size(); k++) {
        cv::circle(img_with_ypr, centers_2d[k], 5, {0, 255, 0}, 2);
      }
      tools::draw_text(img_with_ypr, "FOUND SCALE GRID!", {40, 300}, {0, 255, 0});
    } else {
      tools::draw_text(img_with_ypr, "SEARCHING GRID (STRICT)...", {40, 300}, {0, 0, 255});
    }

    std::string hint = (detect_success) ? "READY! PRESS S" : "ADJUST BOARD!";
    tools::draw_text(img_with_ypr, hint, {40, 340}, detect_success ? cv::Scalar(0, 255, 0) : cv::Scalar(0, 0, 255));
