...
| `Gimbal::send()` 参数 | 来源 |
|-----------------------|------|
| `linear_x` | `/cmd_vel.linear.x`，QY 下行帧以 `float32` 写入负值 |
| `linear_y` | `/cmd_vel.linear.y`，QY 下行帧以 `float32` 写入负值 |
| `angular_z` | `/cmd_vel.angular.z`，QY 下行帧以 `float32` 写入负值 |

注意：当前 `Gimbal::send()` 会把底盘速度限幅到 `[-1, 1]` 后取反，并以小端 IEEE-754 `float32` 写入 QY 的 32 bit 字段。如果导航输出是 m/s、rad/s，需要确保电控端和视觉端对限幅/归一化的理解一致。
