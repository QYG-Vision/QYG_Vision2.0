  ...
  bool to_now)
{
  ...
  double bullet_speed = 24.0; // 统一写死子弹速度，因底层不再发 bullet_speed
  ...
  if (get_send_angle(target, future, bullet_speed, to_now, yaw, pitch)) {
    ...
    if (plan.control) {
      if (first_in_aimer_) {
        ...
        plan.yaw_acc = (tools::limit_rad(yaw - gs.yaw_imu) - tools::limit_rad(gs.yaw_imu - last_yaw_mpc)) /
                       std::pow(dt, 2);
        ...
        plan.pitch_acc = (-pitch - gs.pitch_imu - (gs.pitch_imu + last_pitch_mpc)) / std::pow(dt, 2);
