  ...
  omni_target_priority_{ArmorPriority::fifth}
{
  ...
  // 读取 EKF 参数
  if (yaml["ekf_v1"].IsDefined()) {
    ekf_params_.v1 = yaml["ekf_v1"].as<double>();
    ekf_params_.v2 = yaml["ekf_v2"].as<double>();
    ekf_params_.v1_outpost = yaml["ekf_v1_outpost"].as<double>();
    ekf_params_.v2_outpost = yaml["ekf_v2_outpost"].as<double>();
    Eigen::Vector4d R_dig{
      yaml["ekf_r_yaw"].as<double>(),
      yaml["ekf_r_pitch"].as<double>(),
      yaml["ekf_r_distance"].as<double>(),
      yaml["ekf_r_angle"].as<double>()};
    ekf_params_.R_dig = R_dig;
  }
}
...
bool Tracker::set_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
{
  ...
  target_.set_ekf_params(ekf_params_);
  return true;
}

bool Tracker::update_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
//进行target_更新并进行是否识别到对象的判断
{
  target_.predict(t);

  auto best_center_error = 1e10;
  Armor * best_armor = nullptr;
  for (auto & armor : armors) {
    if (armor.name != target_.name || armor.type != target_.armor_type) continue;

    cv::Point2f img_center(1440 / 2, 1080 / 2);  // TODO
    auto center_error = cv::norm(armor.center - img_center);
