...
std::tuple<std::list<Armor>, std::chrono::steady_clock::time_point> MultiThreadDetector::pop()
{
  ...
  auto y_scale = static_cast<double>(640) / img.cols;
  auto scale = std::min(x_scale, y_scale);
  // armors 返回检测到的装甲板清单，此阶段仅包含 2D 检测结果：
  // 1. 身份：name(编号), color(颜色), type(大/小装甲板)
  // 2. 图像：points(四个角点像素坐标), confidence(置信度)
  // 注意：xyz_in_gimbal、R_armor2camera 等 3D 属性由后续 solver 阶段填充
