...
7. 串口设置
    1. 授予权限
        ...
        ```
        修改后需要**重新登录**或执行 `newgrp dialout` 使权限生效。
    2. 查找串口设备
        ```
        # 查看所有串口设备
        ...
        lsusb
        ```
        找到你的 USB 转串口设备对应的设备名（如 `/dev/ttyUSB0` 或 `/dev/ttyACM0`）。
    3. 获取设备 ID（idVendor 和 idProduct）
        ```
        udevadm info -a -n /dev/ttyUSB0 | grep -E '(ATTRS{idVendor}|ATTRS{idProduct})'
        ```
        将 `/dev/ttyUSB0` 替换为步骤2中找到的实际设备名。
        ...
        **重要提示**：
        - **优先只使用 `idVendor` 和 `idProduct`**，不使用 `serial`
        - `serial` 字段可能不在设备层级（可能在 USB 控制器层级），使用会导致规则无法匹配
        - 如果确实需要使用 `serial`，需要确认它在 `SUBSYSTEM=="tty"` 层级，而不是 `SUBSYSTEMS=="usb"` 层级
    4. 创建 udev 规则文件
        ```
        sudo nano /etc/udev/rules.d/99-usb-serial.rules
        ```
        然后在文件中写入如下内容（**用步骤3获取的真实值替换示例值**）：
        ```
        # USB转串口设备 - 创建 gimbal 符号链接
        # 格式：SUBSYSTEM=="tty", ATTRS{idVendor}=="厂商ID", ATTRS{idProduct}=="产品ID", SYMLINK+="gimbal", MODE="0666", GROUP="dialout"
        SUBSYSTEM=="tty", ATTRS{idVendor}=="1a86", ATTRS{idProduct}=="7523", SYMLINK+="gimbal", MODE="0666", GROUP="dialout"
        ```
        
        **字段说明**：
        - `idVendor` 和 `idProduct`：USB 设备的厂商 ID 和产品 ID（十六进制，小写，必须与步骤3的输出完全一致）
        - `SYMLINK+="gimbal"`：创建符号链接 `/dev/gimbal` 指向该设备
        - `MODE="0666"`：设置设备权限为所有用户可读写
        - `GROUP="dialout"`：设置设备组为 dialout（串口设备组）
    5. 重新加载 udev 规则
        ```
        sudo udevadm control --reload-rules
        sudo udevadm trigger
        ```
        如果设备已连接，`udevadm trigger` 会立即应用规则。
    6. 验证配置
        ```
        # 检查符号链接是否创建
        ls -l /dev/gimbal
        # 预期输出（示例）：
        # lrwxrwxrwx 1 root root 7 Feb  3 21:06 /dev/gimbal -> ttyUSB0
        
        # 测试规则匹配（可选，用于调试）
        sudo udevadm test /sys/class/tty/ttyUSB0 2>&1 | grep -i "gimbal\|symlink"
        # 应该能看到 "LINK 'gimbal'" 的输出
        ```
        
        **故障排查**：
        - 如果 `/dev/gimbal` 不存在，检查：
          1. 规则文件语法是否正确（注意引号、等号、逗号）
          2. `idVendor` 和 `idProduct` 是否与设备完全匹配（大小写、格式）
          3. 设备是否已连接
          4. 执行 `sudo udevadm trigger` 后重新检查
        - 如果权限不足，确认已执行步骤1并重新登录
