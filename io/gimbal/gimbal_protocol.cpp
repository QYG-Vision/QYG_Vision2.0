...
constexpr uint16_t kCrc16X25Table[256] = {
...
{
  GimbalState state;
  state.current_mode = frame.current_mode;
  state.actual_vx = frame.actual_vx;
  state.actual_vy = frame.actual_vy;
  state.actual_wz = frame.actual_wz;
  state.imu_yaw = frame.imu_yaw;
  state.imu_pitch = frame.imu_pitch;
  state.roll_imu = frame.vroll;
